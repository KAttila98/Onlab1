...
class DilatedCausalConv1d(torch.nn.Module):
    ...
    def forward(self, x):
        output = self.conv(x)
        #print(f"shape after DilatedCausalConv1d: {output.shape}")
        return output
...
class CausalConv1d(torch.nn.Module):
    ...
    def forward(self, x):
        output = self.conv(x)
        #print(f"shape after CausalConv1d: {output[:, :, :-1].shape}")
...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, x, skip_size):
        ...
        #print(f"shape after ResidualBlock: {output.shape}")
        #print(f"skip shape after ResidualBlock: {skip.shape}")
        return output, skip
...
class ResidualStack(torch.nn.Module):
    ...
    def forward(self, x, skip_size):
        ...
        #print(f"shape after ResidualStack: {torch.stack(skip_connections).shape}")
...
class DensNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #print(f"shape after DensNet: {output.shape}")
        return output


class WaveNet(torch.nn.Module):
    def __init__(self, layer_size, stack_size, n_tokens, in_channels, res_channels):
        """
        Stack residual blocks by layer and stack size
        :param layer_size: integer, 10 = layer[dilation=1, dilation=2, 4, 8, 16, 32, 64, 128, 256, 512]
        :param stack_size: integer, 5 = stack[layer1, layer2, layer3, layer4, layer5]
        ...
        self.embedding = torch.nn.Embedding(n_tokens, in_channels)
        ...
        self.decoder = torch.nn.Linear(in_channels, n_tokens)
    ...
    def calc_receptive_fields(layer_size, stack_size):
        ...
        return int(num_receptive_fields) + 1 # nem volt +1

    def calc_output_size(self, x):
        output_size = int(x.size(2)) - self.receptive_fields + 1 # nem volt +1

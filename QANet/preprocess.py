...
def filter_large_examples(df):
    ...
    for index, row in tqdm(df.iterrows()):
        ctx_tokens = [w.text for w in nlp(row.context, disable=['parser','ner'])]
        if len(ctx_tokens)>400:
            ctx_lens.append(row.name)

        query_tokens = [w.text for w in nlp(row.question, disable=['parser','ner'])]
        if len(query_tokens)>50:
            query_lens.append(row.name)

        ans_tokens = [w.text for w in nlp(row.answer, disable=['parser','ner'])]
...
def build_word_vocab(vocab_text):
    ...
    for sent in tqdm(vocab_text):
        for word in nlp(sent, disable=['parser','ner']):
...
def context_to_ids(text, word2idx):
    ...
    context_tokens = [w.text for w in nlp(text, disable=['parser','ner'])]
...
def question_to_ids(text, word2idx):
    ...
    question_tokens = [w.text for w in nlp(text, disable=['parser','ner'])]
